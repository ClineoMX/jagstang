...
const FormNoteFiller = forwardRef<FormNoteFillerHandle, FormNoteFillerProps>(
  ({ formId, initialValues, onValuesChange, hideFieldsList = false }, ref) => {
    ...
    const [activeFieldIndex, setActiveFieldIndex] = useState<number | null>(
      null
    );
    ...
    useEffect(() => {
      ...
      // initialValues applied once on form load; intentional omit from deps
    ...
    useEffect(() => {
      ...
      // Parent may pass an unstable callback; avoid re-firing on identity churn
      // eslint-disable-next-line react-hooks/exhaustive-deps
    ...
    const saveFieldValue = () => {
      ...
      setValues((prev) => ({
        ...prev,
        [String(activeFieldIndex)]: modalValue,
      }));
      onClose();
    };
    ...
    const activeFieldType = activeField
      ? normalizeType(activeField.type)
      : 'text';

    const filledCount = fields.filter(
      (_, i) => (values[String(i)] ?? '').trim() !== ''
    ).length;
      ...
      <Box>
        ...
        <List spacing={2}>
          {fields.map((field, index) => {
            ...
            const FieldIcon =
              FIELD_TYPE_ICONS[field.type.toUpperCase()] ?? FiType;
            return (
              <ListItem
                key={index}
                p={3}
          ...
          {pdfBlobUrl ? (
                  ...
                  Array.from({ length: numPages }, (_, pageIdx) => (
                    ...
                    >
                      ...
                      {fields
                        ...
                        .filter(
                          ({ field }) => field.position?.pageIndex === pageIdx
                        )
                        .map(({ field, index }) => {
                          const pos = field.position!;
                          const filled =
                            (values[String(index)] ?? '').trim() !== '';
                          ...
                          return (
                            <Box
                              ...
                              borderWidth="2px"
                              borderColor={
                                filled ? filledFieldBorder : 'brand.400'
                              }
                              bg={bg}
                              opacity={0.9}
                              borderRadius="sm"
            ...
            >
              <Text color={mutedColor}>
                Este formulario no tiene un PDF asociado.
              </Text>
            </Box>
          )}
                ...
                <FormControl>
                  ...
                  {activeFieldType === 'checkbox' ? (
                    <ChakraCheckbox
                      ...
                      onChange={(e) =>
                        setModalValue(e.target.checked ? 'true' : 'false')
                      }
                      size="lg"
                      colorScheme="brand"
                    >
