...
const FormulariosEditorView: React.FC<FormulariosEditorViewProps> = ({
  ...
  const pdfReadyToShow = Boolean(
    pdfSource && pdfContainerWidth > 0 && numPages != null
  );
  ...
  const handleSaveForm = async () => {
    if (!template.name?.trim()) {
      toast({
        ...
        description: 'Ingresa un nombre para el formulario',
        status: 'warning',
        duration: 3000,
      });
      return;
    ...
    if (!pdfFile) {
      toast({
        title: 'PDF requerido',
        description: 'Sube un PDF para guardar el formulario',
        status: 'warning',
        duration: 3000,
      });
      return;
    ...
    if (template.fields.length === 0) {
      toast({
        ...
        description: 'Agrega al menos un campo al formulario',
        status: 'warning',
        duration: 3000,
      });
      return;
    ...
    try {
      ...
      await apiService.createDoctorForm(
        pdfFile,
        template.name.trim(),
        fieldsForApi
      );
      ...
      >
        <AlertDialogOverlay>
          <AlertDialogContent>
            ...
            <AlertDialogBody>
              ¿Estás seguro de que deseas eliminar este formulario? Esta acción
            ...
            <Box>
              {pdfSource ? (
                ...
                >
                  <VStack spacing={4}>
                    <Icon
                      as={isDragging ? FiUpload : FiFileText}
                      boxSize={12}
                      color={isDragging ? 'brand.400' : 'paper.400'}
                    />
                    <Text
                      color={isDragging ? 'brand.fg' : labelColor}
                      textAlign="center"
                    ...
                    <HStack spacing={3} px={4} py={3} align="flex-start">
                      ...
                      <Box minW={0}>
                        ...
                        <Text fontSize="12.5px" color="text.body" mt="2px">
                          Agrega al menos un campo para usar este template como
                          formulario.
                        </Text>
                      </Box>
                    </HStack>
...
const FormulariosEditor: React.FC<FormulariosEditorProps> = ({
  ...
  return <FormulariosEditorView formId={editingFormId} onBack={onBack} />;
};
